...
public class Html2MD {
    ...
    private static String handleInnerHtml(String innerHTML, Element child) {
        ...
        if ("strong".equals(s)) {
            ...
            // 如果这里是 https://img-blog.csdnimg.cn/2d3dadd172a746f4a073f3e53005471a.png
            // 那么我就保存一下文件名，成为本地目录
            URL url = URLUtil.toUrlForHttp(src);
            String name = FileUtil.getName(src);
            File file = FileUtil.touch("./_posts/"+ThreadLocalUtil.get() + File.separator +name );
            // 会自动关闭流

            FileUtil.writeFromStream(url.openConnection().getInputStream(),file);

            innerHTML = "\n" + innerHTML.replace(child.toString(), "![img](" + ThreadLocalUtil.get() + File.separator +name + ")");
        } else if ("em".equals(s)) {
            innerHTML = innerHTML.replace(child.toString(), " *" + child.text() + "* ");
        } else if ("a".equals(s)) {
